...
export default function PlansPage() {
  ...
  const plans = [
    ...
    },
    {
      ...
      actionLink: "/Checkout?total=25000&description=Abonnement Auto Mensuel",
      isCurrent: false,
      popular: true,
    },
    {
    ...
    },
    {
      ...
      actionLink: "/Checkout?total=25000&description=Abonnement Agence Mensuel",
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto mb-8">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-slate-600 hover:text-black transition font-bold"
        >
      ...
      <div className="flex justify-center mb-16">
        <div className="bg-white p-1.5 rounded-2xl border border-slate-200 inline-flex relative shadow-sm">
          <button
            onClick={() => setIsAnnual(false)}
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all relative z-10 ${!isAnnual
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all relative z-10 flex items-center gap-2 ${isAnnual
              ? 'bg-slate-900 text-white shadow-md'
              : 'text-slate-600 hover:text-black'
              }`}
          >
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        {plans.map((plan, index) => (
          <div
            key={index}
            className={`
                relative bg-white rounded-[2.5rem] p-6 shadow-xl border transition-all duration-300 hover:-translate-y-2 flex flex-col
                ${plan.popular ? 'border-purple-500 ring-4 ring-purple-500/10' : 'border-slate-100'}
            `}
          >
            {plan.popular && (
            ...
            <p className="text-slate-600 text-sm mb-6 min-h-[40px] leading-relaxed font-medium">{plan.description}</p>

            <div className="mb-6">
              <span className="text-3xl font-black text-slate-900 tracking-tight">{plan.price}</span>
              {plan.currency && <span className="text-slate-900 text-lg font-bold"> {plan.currency}</span>}
              {plan.period && <span className="text-slate-500 text-xs font-bold"> {plan.period}</span>}
            </div>
            ...
            <ul className="space-y-3 mb-8 flex-1">
              ...
              {plan.notIncluded.map((feature, i) => (
                <li key={i} className="flex items-start gap-3 opacity-50">
                  <div className="mt-0.5 w-4 h-4 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center flex-shrink-0">
                    <X size={10} strokeWidth={3} />
                  </div>
                  <span className="text-slate-500 text-xs font-medium">{feature}</span>
                </li>
              ))}
            </ul>
      ...
      <div className="max-w-3xl mx-auto mt-20 text-center bg-blue-50 rounded-[2.5rem] p-10 border border-blue-100">
        <h3 className="text-2xl font-bold text-slate-900 mb-4">Une question sur les abonnements ?</h3>
